...
export const CommunitySection = (): JSX.Element => {
  ...
  return (
    <section className="flex flex-col items-center gap-8 px-4 py-8 w-full">
      ...
      <Card className="w-full rounded-[32px] overflow-hidden [background:radial-gradient(50%_50%_at_50%_100%,rgba(89,50,158,1)_0%,rgba(39,12,85,1)_100%)] border-0">
        <CardContent className="flex flex-col gap-8 p-8 pb-0">
          <img className="w-[223.08px] h-10" alt="Logo" src="/logo.png" />

          <div className="w-full max-w-[297px] flex flex-col gap-4">
            {benefitsList.map((benefit, index) => (
              <div key={index} className="flex items-start gap-2 w-full">
                <div className="w-6 h-6 flex items-center justify-center">
                  <CheckIcon className="w-[18px] h-3 text-white" />
                </div>
          ...
          <Button className="p-4 bg-[#ffdb3e] text-[#191400] font-bold text-lg rounded-lg shadow-[inset_0px_2px_0px_#ffffff40] hover:bg-[#ffdb3e]/90 w-fit">
            Become a member
          </Button>

          <img
            className="w-[329px] h-[376px] self-end mr-[-32px]"
            alt="New PLATO"
            src="/new-plato-3.png"
          />
        </CardContent>
